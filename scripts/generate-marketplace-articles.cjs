...
function isEngineOwnedArticleDirectory(directory) {
  ...
  const legacySignature = page.includes(`https://marketplace.aproposgroupllc.com/articles/${slug}/`) &&
    page.includes('APROPOS Business Intelligence Marketplace Editorial Team') &&
...
function nav() {
  return `<nav class="site-nav" aria-label="Primary"><a class="brand" href="/"><span class="mark">AMM</span><span class="brand-name">APROPOS Business Intelligence Marketplace<small>APROPOS GROUP LLC</small></span></a><div class="nav-links"><a href="/articles/">Articles</a><a href="/federal-contract-opportunities/">Federal</a><a href="/state-local-contract-opportunities/">State &amp; local</a><a href="/business-contract-readiness/">Business readiness</a><a href="https://aproposgroupllc.com/">APROPOS Group LLC</a></div></nav>`;
}

function footer() {
  return `<footer class="site-footer"><div class="wrap"><div class="footer-grid"><div><span class="mark">AMM</span><h3>APROPOS Business Intelligence Marketplace</h3><p>Public education, service discovery, and routing by APROPOS Group LLC.</p></div><div class="footer-links"><a href="/articles/">Articles</a><a href="/government-contract-intelligence/">Contract Intelligence</a><a href="/federal-contract-opportunities/">Federal Opportunities</a><a href="/state-local-contract-opportunities/">State &amp; Local Opportunities</a><a href="/business-contract-readiness/">Business Readiness</a><a href="https://aproposgroupllc.com/">APROPOS Group LLC</a></div></div><p class="legal">© 2026 APROPOS Group LLC. The Marketplace is not affiliated with or endorsed by any government agency. Educational content does not guarantee eligibility, responsiveness, selection, funding, or contract award.</p></div></footer>`;
}

const corporateNode = { '@type':['Organization','Corporation'], '@id':CORPORATE_ORG, name:'APROPOS Group LLC', url:'https://aproposgroupllc.com/' };
const editorialNode = { '@type':'Organization', '@id':EDITORIAL_TEAM, name:'APROPOS Business Intelligence Marketplace Editorial Team', url:`${SITE}/articles/`, parentOrganization:{'@id':CORPORATE_ORG} };
...
function renderIndex() {
  ...
  const schema = { '@context':'https://schema.org', '@graph':[
    ...
    { '@type':'BreadcrumbList', itemListElement:[{'@type':'ListItem',position:1,name:'APROPOS Business Intelligence Marketplace',item:`${SITE}/`},{'@type':'ListItem',position:2,name:'Articles',item:canonical}] },
...
function renderArticle(article) {
  ...
  const schema = { '@context':'https://schema.org', '@graph':[
    ...
    { '@type':'BreadcrumbList', itemListElement:[{'@type':'ListItem',position:1,name:'APROPOS Business Intelligence Marketplace',item:`${SITE}/`},{'@type':'ListItem',position:2,name:'Articles',item:`${SITE}/articles/`},{'@type':'ListItem',position:3,name:article.title,item:canonical}] }
  ]};
  ...
  const html = `<!doctype html>${ENGINE_MARKER}<html lang="en"><head>${sharedHead(`${article.title} | APROPOS Marketplace`,article.description,canonical,'article',extra)}</head><body><a class="skip" href="#main">Skip to main content</a>${nav()}<main id="main"><header class="hero"><div class="wrap"><div class="breadcrumbs"><a href="/">Marketplace</a> / <a href="/articles/">Articles</a> / ${esc(article.title)}</div><div class="eyebrow">${esc(article.category)} · APROPOS BUSINESS INTELLIGENCE MARKETPLACE</div><h1>${esc(article.title)}</h1><p class="lead">${esc(article.description)}</p><div class="article-meta"><span>Published ${esc(article.publishedDate)}</span><span>Updated ${esc(article.updatedDate)}</span><span>By APROPOS Business Intelligence Marketplace Editorial Team</span></div></div></header>${sections}${sources}${pathway}${relatedHtml}</main>${footer()}${analytics}</body></html>`;
