...
const MARKETPLACE_ORG = `${SITE}/#organization`;
...
for (const article of articles) {
  ...
  for (const key of required) if (!article[key] || (key === 'sections' && !Array.isArray(article[key]))) throw new Error(`[articles] ${article.__file}: missing ${key}`);
...
function renderArticle(article) {
  ...
  const sources = article.sources?.length ? `<section><div class="wrap article-body"><h2>Sources and further reading</h2><ul class="source-list">${article.sources.map((source)=>`<li><a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${esc(source.label)}</a></li>`).join('')}</ul></div></section>` : '';
  ...
  const schema = { '@context':'https://schema.org', '@graph':[
    { '@type':'Article', '@id':`${canonical}#article`, headline:article.title, description:article.description, datePublished:article.publishedDate, dateModified:article.updatedDate, mainEntityOfPage:{'@id':`${canonical}#webpage`}, author:{'@type':'Organization','@id':MARKETPLACE_ORG,name:'APROPOS Marketing Marketplace Editorial Team'}, publisher:{'@id':MARKETPLACE_ORG}, about:article.category, image:`${SITE}/og-marketplace.jpg` },
    { '@type':'WebPage', '@id':`${canonical}#webpage`, url:canonical, name:article.title, isPartOf:{'@id':`${SITE}/#website`}, publisher:{'@id':MARKETPLACE_ORG}, primaryImageOfPage:{'@type':'ImageObject',url:`${SITE}/og-marketplace.jpg`} },
...
function patchNavigation() {
  ...
  if (!homepage.includes('href="/articles/"')) {
    const marker = '<a href="/contractor-participation/">Participate</a>';
    if (!homepage.includes(marker)) throw new Error('[articles] homepage navigation marker missing');
    homepage = homepage.replace(marker, `${marker}\n    <a href="/articles/">Learn</a>`);
  ...
  for (const route of landingPages) {
    ...
    if (!page.includes('href="/articles/"')) {
      const marker = '<a href="/contractor-participation/">Participate</a>';
      if (page.includes(marker)) page = page.replace(marker, `${marker}<a href="/articles/">Learn</a>`);
...
function updateSitemap() {
  ...
  const latest = articles.map((a)=>a.updatedDate).sort().at(-1) || '2026-08-21';
...
patchNavigation();
updateSitemap();
console.log(`[articles] generated article index and ${articles.length} canonical article page(s); navigation and sitemap updated`);
